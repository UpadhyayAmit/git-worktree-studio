...
function activate(context) {
    ...
    const manager = new gitWorktreeManager_1.GitWorktreeManager(workspaceFolders);
    const reposProvider = new treeProvider_1.ReposTreeProvider(manager);
    const changesProvider = new treeProvider_1.ChangesTreeProvider(manager);
    // Register tree views
    ...
    async function requireInput(prompt, placeholder = '') {
        return vscode.window.showInputBox({ prompt, placeHolder: placeholder, ignoreFocusOut: true });
    }
    // -------------------------------------------------------------------------
    // Worktree Management Commands
    // -------------------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.createWorktree', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.addBranch', async (item) => {
        if (!item?.repoPath) {
            return;
        }
        const branchName = await requireInput('Enter branch name', 'feature/my-feature');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.removeWorktree', async (item) => {
        if (!item?.repoPath || !item.worktreePath) {
            return;
        }
        const confirm = await vscode.window.showWarningMessage(`Remove worktree for branch "${item.branchName}"? This will delete the directory.`, { modal: true }, 'Remove');
        if (confirm !== 'Remove') {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.openWorktree', async (item) => {
    ...
    // -------------------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.commitWorktree', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.push', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.pull', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
        try {
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Pulling…' }, () => manager.pull(item.worktreePath));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.syncBranch', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
        try {
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Syncing…' }, () => manager.sync(item.worktreePath));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.createPR', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
        ...
        try {
            ...
            const openBrowser = await vscode.window.showInformationMessage(`PR created: ${result}`, 'Open in Browser');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.stash', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.stashPop', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.mergeBranch', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.viewLog', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.switchBranch', async (item) => {
        if (!item?.worktreePath) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.deleteBranch', async (item) => {
        if (!item?.repoPath || !item.branchName) {
            return;
        }
        const confirm = await vscode.window.showWarningMessage(`Delete branch "${item.branchName}"?`, { modal: true }, 'Delete');
        if (confirm !== 'Delete') {
            return;
        }
        try {
    ...
    // -------------------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.stageFile', async (item) => {
        if (!item?.worktreePath || !item.filePath) {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.unstageFile', async (item) => {
        if (!item?.worktreePath || !item.filePath) {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.discardFile', async (item) => {
        if (!item?.worktreePath || !item.filePath) {
            return;
        }
        const confirm = await vscode.window.showWarningMessage(`Discard changes to "${item.filePath}"?`, { modal: true }, 'Discard');
        if (confirm !== 'Discard') {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.diffFile', async (item) => {
        if (!item?.worktreePath || !item.filePath) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.openFile', async (item) => {
        if (!item?.worktreePath || !item.filePath) {
            return;
        }
        const absolutePath = path.join(item.worktreePath, item.filePath);
    ...
    // Global Commands
    // -------------------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.refresh', () => refreshAll()));
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.fetchAll', async () => {
        const repos = await manager.discoverRepos();
        try {
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Fetching all remotes…' }, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gitMultiBranch.startMcpServer', () => {
    ...
    const onFsChange = () => {
        ...
        debounceTimer = setTimeout(() => refreshAll(), 2000);
    };
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeWorkspaceFolders(e => {
        const folders = (e.added.concat(vscode.workspace.workspaceFolders?.filter(f => !e.removed.some(r => r.uri.fsPath === f.uri.fsPath)) ?? [])).map(f => f.uri.fsPath);
